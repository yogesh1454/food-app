 ...
 * 
 * Custom queries for menu item search with geospatial filtering and blended ranking
 */
@Repository
public interface SearchMenuItemRepository extends JpaRepository<SearchMenuItem, Long> {
    ...
    @Query(value = """
        ...
        WHERE ST_DWithin(
            mi.branch_location::geography,
            ST_SetSRID(ST_MakePoint(:longitude, :latitude), 4326)::geography,
            :radiusMeters
        )
        AND mi.is_available = true
        AND mi.is_deleted = false
        AND mi.order_count_30d > 10
