...
public class SessionManagementService {
    ...
    public String createSession(CheckoutSession session) {
        ...
        session.setStatus(CheckoutSessionStatus.READY_FOR_COMMIT);

        String key = SESSION_KEY_PREFIX + sessionId;

        // Check if session already exists (idempotency)
        CheckoutSession existing = getSession(sessionId).orElse(null);
        if (existing != null && existing.getStatus() == CheckoutSessionStatus.READY_FOR_COMMIT) {
    ...
    public CheckoutSession lockSession(String sessionId) {
        ...
        if (session.getStatus() == CheckoutSessionStatus.COMMITTED) {
        ...
        session.setStatus(CheckoutSessionStatus.IN_PROGRESS);
        String key = SESSION_KEY_PREFIX + sessionId;
        redisTemplate.opsForValue().set(key, session, SESSION_TTL);
     ...
     */
    public void updateSessionStatus(String sessionId, CheckoutSessionStatus status) {
     ...
     */
    public void updateSessionStatus(String sessionId, CheckoutSessionStatus status, String orderId) {
        Optional<CheckoutSession> sessionOpt = getSession(sessionId);
        ...
        if (sessionOpt.isPresent()) {
            ...
            if (status == CheckoutSessionStatus.COMMITTED) {
    ...
    public boolean isSessionValid(String sessionId) {
        return getSession(sessionId)
                .map(session -> session.getStatus() == CheckoutSessionStatus.READY_FOR_COMMIT)
                .orElse(false);
    }
}
