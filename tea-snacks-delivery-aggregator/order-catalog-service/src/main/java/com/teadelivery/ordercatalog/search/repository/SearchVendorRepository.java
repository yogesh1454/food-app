...
public interface SearchVendorRepository extends JpaRepository<SearchVendor, Long> {
    ...
    @Query(value = """
            SELECT v.*,
                   ST_Distance(CAST(v.location AS geography), CAST(ST_SetSRID(ST_MakePoint(:longitude, :latitude), 4326) AS geography)) / 1000.0 AS distance_km
            FROM search_vendors v
            WHERE ST_DWithin(
                CAST(v.location AS geography),
