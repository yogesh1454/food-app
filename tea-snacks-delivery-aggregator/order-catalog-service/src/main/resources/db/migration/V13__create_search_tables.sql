...
CREATE TABLE search_vendors (
    ...
    normalized_popularity DECIMAL(5, 4),
...
COMMENT ON COLUMN search_vendors.search_vector IS 'Full-text search vector';
COMMENT ON COLUMN search_vendors.normalized_popularity IS 'Normalized popularity score for blended ranking';
COMMENT ON COLUMN search_vendors.images IS 'JSONB array of image URLs with multiple sizes';
COMMENT ON COLUMN search_menu_items.trending_score IS 'Trending score based on recent orders';
COMMENT ON COLUMN search_menu_items.normalized_popularity IS 'Normalized popularity score for blended ranking';